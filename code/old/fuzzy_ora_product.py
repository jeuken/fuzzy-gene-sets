...
def compute_fuzzy_intersection(query_set, pathway_genes, pathway_memberships):
    """Calculates the (fuzzy) intersection size between query set and a pathway, handling NaN/Inf values."""
    intersection = 0
    for gene, membership in zip(pathway_genes, pathway_memberships):
        query_value = query_set.get(gene, 0)
        # Replace NaN or Inf values with 0
        query_value = 0 if not np.isfinite(query_value) else query_value
        membership = 0 if not np.isfinite(membership) else membership
        intersection += query_value * membership
    return intersection
...
output_file = "/Users/piamozdzanowski/VU/Fuzzy_Gene_Sets/data/ORA_output/Alzheimers_GSE95587_product_t/ora_scores.csv"
output_dir = "/Users/piamozdzanowski/VU/Fuzzy_Gene_Sets/data/ORA_output/Alzheimers_GSE95587_product_t/null_distributions"
