...
def process_and_expand_pathways(gmt_path, Ensembl_ID_dict_path, string_scores_path, output_file):
    ...
    try:
        ...
        # Create a cumulative distribution for gene frequencies
        gene_counts_sorted = gene_counts.sort_values(ascending=False).reset_index()
        gene_counts_sorted.columns = ['Ensembl_ID', 'Frequency']
        gene_counts_sorted['Cumulative_Distribution'] = gene_counts_sorted['Frequency'].rank(pct=True)

        # Map cumulative distribution to the original DataFrame
        pathways_df = pathways_df.merge(
            gene_counts_sorted[['Ensembl_ID', 'Cumulative_Distribution']],
            on='Ensembl_ID',
            how='left'
        )

        # Calculate overlap membership as 1 minus the cumulative distribution
        pathways_df['Overlap_Membership'] = 1 - pathways_df['Cumulative_Distribution']

        # Ensure NaNs are handled (if there are any genes without a cumulative distribution)
        pathways_df['Overlap_Membership'].fillna(0, inplace=True)
