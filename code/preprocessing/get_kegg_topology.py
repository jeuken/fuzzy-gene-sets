...
def kegg_topology_all(kgml_dir, output_dir, plot=False):
    """
    Process all KGML files in a directory to compute centrality metrics for each pathway.

    Parameters:
    - kgml_dir (str): Directory containing KGML files.
    - output_dir (str): Directory to save output files and plots.
    - plot (bool): Whether to generate and save plots of the pathway networks.

    Returns:
    - DataFrame: Combined centrality metrics for all pathways processed.
    """
    # Initialize an empty DataFrame to hold centrality metrics for all pathways
    kegg_centrality_df = pd.DataFrame(columns=['Pathway_Name', 'Description', 'genes', 'Degree', 'In_Degree', 'Out_Degree', 'Closeness', 'Local_Reaching', 'Betweenness'])
    
    # List all .kgml files in the specified directory
    kgml_files = [os.path.join(kgml_dir, file) for file in os.listdir(kgml_dir) if file.endswith('.kgml')]
    
    # Define a function to process a single KGML file and return its gene centrality DataFrame
    def process_file(kgml_file):
        gene_centrality_df, _, _ = kegg_topology(kgml_file, output_dir, plot)
        return gene_centrality_df

    # Process each KGML file in parallel and track progress using tqdm
    ...
    # Save the combined DataFrame of centrality metrics to a CSV file
    output_file = os.path.join(output_dir, 'kegg_centrality_results.csv')
    kegg_centrality_df.to_csv(output_file, index=False)
    
    return kegg_centrality_df  # Return the DataFrame containing centrality metrics for all pathways


# Example for all pathways
output_dir = '../../data/pathways/KEGG/topology/'
kgml_dir = '../../data/pathways/KEGG/kgml/'
centrality_df = kegg_topology_all(kgml_dir, output_dir, plot=True)


# Example for one pathway
#kgml_file = '../../data/pathways/KEGG/kgml/hsa05414.kgml'
#gene_centrality_df,nodes,edges = kegg_topology(kgml_file,output_dir, plot=True)
