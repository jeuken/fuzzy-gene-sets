...
def process_kgml_file(kgml_file):
    ...
    if not relations.empty:
        ...
        pprel_rels = relations[relations['@type'] == 'PPrel']
        gerel_rels = relations[relations['@type'] == 'GErel']  # Add GErel filtering
        ...
        for _, pprel in pprel_rels.iterrows():
            ...
            # Handle gene-group relations (gene to group)
                ...
                gene_gene = pd.concat([gene_gene, pd.DataFrame([[gene1_name, group_gene_name_str, 'gene_to_group']],
                                                               columns=['gene1', 'gene2', 'subtype'])], ignore_index=True)
            ...
            # Handle group-gene relations (group to gene)
                ...
                gene_gene = pd.concat([gene_gene, pd.DataFrame([[group_gene_name_str, id_to_name.get(entry2, entry2), 'group_to_gene']],
                                                               columns=['gene1', 'gene2', 'subtype'])], ignore_index=True)
            ...
            # Handle group-group relations (group to group)
                ...
                gene_gene = pd.concat([gene_gene, pd.DataFrame([[group1_gene_name_str, group2_gene_name_str, 'group_to_group']],
                                                               columns=['gene1', 'gene2', 'subtype'])], ignore_index=True)

        # Now apply the same logic for GErel relations
        for _, gerel in gerel_rels.iterrows():
            entry1, entry2 = gerel['@entry1'], gerel['@entry2']
        
            if entry1 in gene_ids and entry2 in gene_ids:
                gene1_name = id_to_name.get(entry1, entry1)
                gene2_name = id_to_name.get(entry2, entry2)
                subtype = extract_subtype(gerel.get('subtype'))
                gene_gene = pd.concat([gene_gene, pd.DataFrame([[gene1_name, gene2_name, subtype]],
                                                               columns=['gene1', 'gene2', 'subtype'])], ignore_index=True)
        ...
        for _, ecrel in ecrel_rels.iterrows():
            ...
            if entry1 in genes['@id'].values and entry2 in genes['@id'].values:
                ...
                for group_gene_id in group_gene_ids:
                    ...
                    gene_gene = pd.concat([gene_gene, pd.DataFrame([[gene1_name, group_gene_name, subtype]],
                                                                    columns=['gene1', 'gene2', 'subtype'])], ignore_index=True)
            ...
            # Check if entry1 is a group and entry2 is a gene
                ...
                for group_gene_id in group_gene_ids:
                    group_gene_name = id_to_name.get(group_gene_id, group_gene_id)
                    gene2_name = id_to_name.get(entry2, entry2)
                    new_entries = pd.DataFrame([[group_gene_name, gene2_name, subtype]],
            ...
            # Check if both entry1 and entry2 are groups
                ...
                for g1 in group1_gene_ids:
                    for g2 in group2_gene_ids:
                        ...
                        new_entries = pd.DataFrame([[g1_name, g2_name, subtype]],
                                                   columns=['gene1', 'gene2', 'subtype'])
                        gene_gene = pd.concat([gene_gene, new_entries], ignore_index=True)
